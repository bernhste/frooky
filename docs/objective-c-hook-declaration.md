...
- `retType`: Optional return type of the Objective-C method.
...
You can provide that information by declaring `retType` and or `params` for each method. In this form, `name` must include the Objective-C method prefix, `-` for instance methods or `+` for class methods.
